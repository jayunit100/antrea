			...
			[]*ofTestUtils.ExpectFlow{
				{
					...
					fmt.Sprintf("dec_ttl,set_field:%s->eth_src,set_field:%s->eth_dst,load:0x%x->NXM_NX_REG1[],load:0x1->NXM_NX_REG0[16],set_field:%s->tun_dst,resubmit(,110)", localGwMAC.String(), vMAC.String(), tunnelPort, peerNodeIP.String())},
			},
		},
	}
}
